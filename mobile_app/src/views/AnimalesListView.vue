  ...
  <AppLayout>
    <PullToRefresh @refresh="onRefresh" :loading="refreshing">
      <div class="welcome-header mb-4 text-start d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
        <div>
          <h2 class="h4 fw-bold mb-1 text-dark">Animales Registrados</h2>
          <p class="text-secondary small mb-0">Administra el inventario de animales en los predios</p>
        </div>
      ...
      <div class="card border-0 shadow-sm p-0 overflow-hidden card-outer-mobile-flat bg-white rounded-4">
        <div class="card-header bg-white p-3 p-md-4 border-bottom border-slate-100">
          <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3 w-100">
            <h5 class="mb-0 fw-bold fs-5 text-dark text-center text-md-start w-100 w-md-auto">
              <i class="bi bi-gender-male text-primary me-2"></i>Inventario Animal
            </h5>
            <div class="w-100 w-md-auto">
              <button @click="$router.push('/animales/nuevo')" class="btn btn-primary btn-sm-custom d-flex align-items-center justify-content-center gap-1.5 px-3 py-2 bg-primary text-white border-0 rounded-3 w-100 w-md-auto">
        ...
        <div class="card-body p-0 text-start">
          <div v-if="!loading && filteredAnimales.length === 0" class="text-center p-5 text-muted">
            <i class="bi bi-gender-male display-6 d-block mb-2 text-muted"></i>
            <p class="mb-0 small text-secondary">No hay animales registrados en el sistema</p>
          </div>
          ...
          <div v-else>
              ...
              <table class="table table-hover align-middle mb-0">
                <thead class="table-light">
                  <tr>
                    ...
                    <th class="text-secondary fw-bold text-uppercase fs-7 tracking-wider">Arete</th>
                    <th class="text-secondary fw-bold text-uppercase fs-7 tracking-wider">Raza</th>
                    <th class="text-secondary fw-bold text-uppercase fs-7 tracking-wider">Sexo</th>
                    <th class="text-secondary fw-bold text-uppercase fs-7 tracking-wider">Edad</th>
                    <th class="text-secondary fw-bold text-uppercase fs-7 tracking-wider">Predio</th>
                    <th class="text-center text-secondary fw-bold text-uppercase fs-7 tracking-wider">Acciones</th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="animal in paginatedAnimales" :key="animal.id" class="border-bottom">
                    <td class="ps-4 text-dark">{{ animal.id }}</td>
                    ...
                    <td class="text-dark">
                      <div v-if="animal.predio">
                        ...
                        <small class="text-secondary">CLU: {{ animal.predio.clave_unidad_produccion || '—' }}</small>
                      </div>
                    ...
                    <td>
                      <div class="d-flex justify-content-center gap-2">
                        <button
                          @click="$router.push('/animales/' + animal.id)"
                          class="btn btn-sm btn-outline-info d-flex align-items-center justify-content-center gap-1.5 px-3 py-1-5 bg-transparent border-info text-info rounded-3"
                          title="Ver animal"
                        >
                          <i class="bi bi-eye"></i> Ver
                        </button>
                        <button
                          @click="$router.push('/animales/editar/' + animal.id)"
            ...
            <div class="mobile-cards d-lg-none">
              <div class="mobile-cards-grid">
                <article v-for="animal in paginatedAnimales" :key="animal.id" class="medico-mobile-card shadow-sm">
                  <div class="medico-content">
                    <div class="field-block">
                      <span class="field-label">ARETE SINIIGA</span>
                      <span class="field-value fw-bold text-dark fs-6">{{ animal.numero_arete_siniiga }}</span>
                    </div>

                    <div class="field-block">
                      <span class="field-label">RAZA</span>
                      <span class="field-value text-dark">{{ animal.raza || '—' }}</span>
                    </div>

                    <div class="field-block">
                      <span class="field-label">SEXO</span>
                      <span class="field-value text-dark">{{ animal.sexo || '—' }}</span>
                    </div>
                    ...
                    <div class="field-block">
                      ...
                      <span class="field-value text-dark" v-if="animal.predio">
                        <span class="fw-semibold">{{ animal.predio.nombre_rancho || 'Sin nombre' }}</span>
                        <small class="text-secondary d-block">CLU: {{ animal.predio.clave_unidad_produccion || '—' }}</small>
                        <small class="text-secondary d-block">Productor: {{ productorName(animal) }}</small>
                      </span>
                      <span v-else class="field-value text-muted">—</span>
                    </div>
                  </div>
...
export default {
  ...
  computed: {
    filteredAnimales() {
      if (!this.search) return this.animales;
    ...
    totalResults() {
      return this.filteredAnimales.length;
    },
    startResult() {
    ...
    paginatedAnimales() {
      ...
      return this.filteredAnimales.slice(start, end);
    }
  },
  watch: {
    search() {
  ...
  async mounted() {
    ...
    if (!this.isAdmin) {
      alert('Solo administradores pueden gestionar animales.');
      this.$router.push('/dashboard');
      return;
    }

    await this.loadAnimales();
  ...
  methods: {
    productorName(animal) {
      if (!animal.predio || !animal.predio.productor) return '—';
      const p = animal.predio.productor;
    ...
    async loadAnimales() {
      ...
      // Offline-first: mostrar cache local inmediatamente
      const cached = await db.getAnimales();
      if (cached.length > 0) {
        this.animales = cached;
      }
      ...
      try {
        ...
        if (res.data) {
          ...
          this.currentPage = 1;
          this.errorMsg = '';
          await db.saveAnimales(res.data);
        }
      } catch (e) {
        if (this.animales.length === 0) {
          this.errorMsg = e.message || 'No se pudieron cargar los animales.';
        }
      } finally {
        this.loading = false;
      }
    ...
    async deleteAnimal(animal) {
      if (!confirm(`¿Estás seguro de eliminar al animal con arete "${animal.numero_arete_siniiga}"?`)) return;
      this.errorMsg = '';
      this.successMsg = '';
      try {
        await api.deleteAnimal(animal.id);
        this.successMsg = `Animal con arete "${animal.numero_arete_siniiga}" eliminado del sistema.`;
        await this.loadAnimales();
      } catch (e) {
        this.errorMsg = e.message || 'No se pudo eliminar el animal.';
      }
    },
    prevPage() {
