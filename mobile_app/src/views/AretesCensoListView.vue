  ...
  <AppLayout>
    <PullToRefresh @refresh="onRefresh" :loading="refreshing">
      <div class="welcome-header mb-4 text-start d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center gap-3">
        <div>
          <h2 class="h4 fw-bold mb-1 text-dark">Aretes del Censo</h2>
          <p class="text-secondary small mb-0">Administra los aretes registrados en el censo pecuario</p>
        </div>
      ...
      <div class="card border-0 shadow-sm p-0 overflow-hidden card-outer-mobile-flat bg-white rounded-4">
        <div class="card-header bg-white p-3 p-md-4 border-bottom border-slate-100">
          <div class="d-flex flex-column flex-md-row justify-content-between align-items-center gap-3 w-100">
            <h5 class="mb-0 fw-bold fs-5 text-dark text-center text-md-start w-100 w-md-auto">
              <i class="bi bi-upc-scan text-primary me-2"></i>Listado de Aretes
            </h5>
            <div class="w-100 w-md-auto">
              <button @click="$router.push('/aretes-censo/nuevo')" class="btn btn-primary btn-sm-custom d-flex align-items-center justify-content-center gap-1.5 px-3 py-2 bg-primary text-white border-0 rounded-3 w-100 w-md-auto">
        ...
        <div class="card-body p-0 text-start">
          <div v-if="!loading && filteredAretes.length === 0 && !search" class="text-center p-5 text-muted">
            <i class="bi bi-upc-scan display-6 d-block mb-2 text-muted"></i>
            <p class="mb-0 small text-secondary">No hay aretes registrados en el sistema</p>
          </div>
          <div v-else-if="!loading && filteredAretes.length === 0 && search" class="text-center p-5 text-muted">
            <i class="bi bi-search display-6 d-block mb-2 text-muted"></i>
            <p class="mb-0 small text-secondary">No se encontraron aretes con el criterio de búsqueda</p>
          </div>
              ...
              <table class="table table-hover align-middle mb-0 text-nowrap" style="min-width: 950px;">
                <thead class="table-light">
                  <tr>
                    ...
                    <th class="text-secondary fw-bold text-uppercase fs-7 tracking-wider">Edad (meses)</th>
                    <th class="text-secondary fw-bold text-uppercase fs-7 tracking-wider">Productor</th>
                ...
                <tbody>
                  <tr v-for="(arete, index) in paginatedAretes" :key="arete.id" class="border-bottom">
                    <td class="ps-4 text-secondary">{{ (currentPage - 1) * perPage + index + 1 }}</td>
                    <td>
                      <div class="d-flex align-items-center">
                        ...
                        <div>
                          <div class="fw-bold text-dark fs-6">{{ arete.numero_arete }}</div>
                          <small class="text-secondary" v-if="arete.fecha_nacimiento">{{ arete.fecha_nacimiento }}</small>
                        </div>
                      </div>
                    </td>
                    ...
                    <td class="text-dark">{{ getProductorName(arete.productor) }}</td>
...
export default {
  ...
  computed: {
    ...
    paginatedAretes() {
      ...
      return this.filteredAretes.slice(start, end);
    }
  },
  watch: {
    search() {
  ...
  async mounted() {
    ...
    if (!this.isAdmin) {
      alert('Acceso restringido. Solo administradores pueden gestionar los aretes del censo.');
  ...
  methods: {
    ...
    async loadAretes() {
      ...
      try {
        ...
        const res = await api.getAretesCenso(params);
        if (res.data) {
        ...
        if (this.aretes.length === 0) {
          this.errorMsg = e.message || 'No se pudieron cargar los aretes del censo.';
        }
      } finally {
        this.loading = false;
      }
    ...
    async deleteAreteCenso(arete) {
      if (!confirm(`¿Estás seguro de eliminar el arete "${arete.numero_arete}"? Esta acción no se puede deshacer.`)) return;
      this.errorMsg = '';
      this.successMsg = '';
      try {
        await api.deleteAreteCenso(arete.id);
        this.successMsg = `Arete "${arete.numero_arete}" eliminado del censo.`;
        await this.loadAretes();
      } catch (e) {
        this.errorMsg = e.message || 'No se pudo eliminar el arete.';
      }
    },
    prevPage() {
